...
const SmartMedicalAdvisor = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Save to database
      if (user) {
        const history = await loadHealthHistory(user.id);
        const previousScore = history.length ? history[history.length - 1].score : null;
        ...
        const sent = await notifyDoctorsIfWorse({
          currentScore: confidenceToScore(d.confidence),
          previousScore,
